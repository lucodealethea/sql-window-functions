...
Imagine writing a query using regular GROUP BY: we would have to use a correlated subquery and a JOIN. 
The query would neither be readable nor efficient. We no longer want to pay that price and PARTITION BY is the solution. 
Thanks to PARTITION BY, we can easily get the information about individual rows AND the information about the groups 
...
----+------------+-------+
 ...
 -- (Using more than 1 column in Partition By) Show the id of each journey, the date on which it took place, 
-- the model of the train that was used, the max_speed of that train and the highest max_speed from all the trains 
-- that ever went on the same route on the same day.
...
* OVER(PARTITION BY x) works in a similar way to GROUP BY, defining the window as all the rows in the query result 
						that have the same value in x.
