'
At the end of this part, we will compare window functions with other elements of SELECT queries. 
We will find out how to use window functions in various parts of the query and where we are not allowed to do so.
'
...
-- we would like to show those countries (country name and average final price) that have the average final 
-- price higher than the average price from all over the world. 
...
-- divide all auctions into 6 equal groups based on the asking_price in ascending order. 
...
'
To sum up this section, we just have to remember the following rule: the only places where we can use window functions 
without having to write subqueries are the SELECT and ORDER BY clauses. In all other places we have to use subqueries.
'


'
Before, we said that window functions were calculated after the GROUP BY clause. This has a very important implication 
for our queries: if the query uses any aggregates, GROUP BY or HAVING, the window function sees the group rows instead of 
...
>>>
ERROR: column "auction.final_price" must appear in the GROUP BY clause or be used in an aggregate function Line: 
3 Position in the line: 19
'
This query does not work. This is because we cant use the column final_price in the window function. 
...
'
The query will now succeed because we used an aggregate function (MAX(final_price)) that was indeed available after grouping the rows. 
By the way, this is the only place where we can nest aggregate functions inside one another.
...
-- Group the auctions by the country. Show the country, the minimal number of participants in an auction and 
		-- the average minimal number of participants across all countries.
...
---------+-------+------+
 ...
 UK      |     3 |    5 |
'
We grouped auctions with respect to the country, counted the number of auctions from each country, 
		and then we created a ranking based on that count of auctions.
'
...
'
Finally, we can use window functions with PARTITION BY on grouped rows. One thing we need to remember is that the window function 
		will only see grouped rows, not the original rows. For example:
'
...
The query might require a bit of explanation. First of all, we grouped all rows by the country and ended. 
		...
		First, we simply sum the views in accordance with our GROUP BY clause, i.e. we get the sum of views in all auctions 
		from the particular country on the particular day. But look what happens next. 
		We use a window function to sum all daily sums for a particular country. 
