'
Typically, OVER() is used to compare the current row with an aggregate. For example, we can compute the difference 
between an employee salary and the average salary
'
...
-- For each employee in table employee, select first and last name, years_worked, average of years spent in the company 
-- by all employees, and the difference between the years_worked and the average as difference 
...
------------+-----------+--------------+--------------------+---------------------+
 ...
 -- (CALCULATING PERCENTAGES) For all employees from department with department_id = 3, 
 -- show their first_name, last_name, salary, the % of their salary to the SUM of all salaries in that department as percentage
SELECT first_name, last_name, salary,  
...
-- For each employee that earns more than 4000, show their first_name, last_name, salary and the number of all employees 
-- who earn more than 4000
select first_name, last_name, salary,
	count(*) OVER() 
from employee
where salary>4000;
...
'
OVER() means "for all rows in the query result". This "in the query result" part is very important 
   – window functions work only on the rows returned by the query.

Here, this means we will get the salary of each IT department employee and the average salary in that department, 
and not in the entire company. That is a very important rule which we need to remember. 
Window functions are always executed AFTER the WHERE clause, so they work on whatever they find as the result.

Now, it might be tempting to use window functions in a WHERE clause. However, when we run a query, 
we will get an error message. We cannot put window functions in WHERE. 
Why? The window functions are applied AFTER the rows are selected. 
If the window functions were in a WHERE clause, we would get a circular dependency: 
in order to compute the window function, we have to filter the rows with WHERE, which requires to compute the window function.
'
